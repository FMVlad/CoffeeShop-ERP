...
def generate_postings(doc_id: int, db: pyodbc.Connection = Depends(get_db)):
    # Перевірка періоду
    head = _fetch_one(db, "SELECT [Date], CenterID, PricesIncludeVAT FROM SalesDocuments WHERE ID=?", (doc_id,))
    ...
    # Зібрати суми
    cur = db.cursor()
    rows = cur.execute("SELECT Quantity, Price FROM SalesDocumentItems WHERE DocID=?", (doc_id,)).fetchall()
    total_gross = sum((Decimal(str(r[0] or 0)) * Decimal(str(r[1] or 0))) for r in rows)
    # ПДВ 20% для прикладу
    vat_rate = Decimal("20")
