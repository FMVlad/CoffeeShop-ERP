  ...
  return (
    <div className="min-h-screen bg-gradient-to-tr from-coffee-50 via-white to-coffee-100 px-4 py-6">
      <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow p-6">
        ...
        <hr className="my-6" />
        <h2 className="text-base font-semibold text-coffee-800 mb-3">Пресет</h2>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          <div>
            <label className="block text-sm text-coffee-600 mb-1">Ключ пресету</label>
            <input className="w-full border rounded px-3 py-2" value={presetKey} onChange={(e)=>setPresetKey(e.target.value)} />
          </div>
          <div>
          ...
          <label className="inline-flex items-center gap-2 text-sm mt-6">
            <input type="checkbox" checked={enabled} onChange={(e)=>setEnabled(e.target.checked)} /> Автозапуск увімкнено
          </label>
          <div className="flex gap-2">
            <button onClick={savePreset} disabled={busy} className="flex-1 bg-coffee-600 hover:bg-coffee-700 text-white rounded px-4 py-2 disabled:opacity-60">Зберегти пресет</button>
            <button onClick={runPreset} disabled={busy} className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded px-4 py-2 disabled:opacity-60">Запустити за пресетом</button>
          </div>
        </div>
